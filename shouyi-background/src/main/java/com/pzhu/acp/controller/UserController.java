...
public class UserController {
    ...
    public BaseResponse<Boolean> userReset(@RequestBody UserResetRequest userResetRequest) {
        if (StringUtils.isBlank(userResetRequest.getEmail()) ||
                StringUtils.isBlank(userResetRequest.getPassword()) ||
                StringUtils.isBlank(userResetRequest.getCode())) {
            log.error("参数校验失败,该参数为：{}", GsonUtil.toJson(userResetRequest));
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        if (userResetRequest.getPassword().length() < 8) {
            log.error("参数校验失败,该参数为：{}", GsonUtil.toJson(userResetRequest));
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户密码过短");
        }
        User user = new User();
        BeanUtils.copyProperties(userResetRequest, user);
        user.setPasssword(userResetRequest.getPassword());
        Boolean isSuccess = userService.userReset(user, userResetRequest.getCode());
        return ResultUtils.success(isSuccess);
    }
