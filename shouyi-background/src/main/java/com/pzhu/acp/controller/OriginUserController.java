...
import com.pzhu.acp.model.dto.QuitOriginRequest;
...
public class OriginUserController {
    ...
    @GetMapping("/oids/{uid}")
    public List<Long> getOidListByUid(@PathVariable Long uid) {
        return originUserService.findOidListByUid(uid);
    }
    @DeleteMapping("/quitOriginUser")
    public BaseResponse<Boolean> quitOriginUser(@RequestBody QuitOriginRequest quitOriginRequest) {
        if (quitOriginRequest.getOid() == 0 && quitOriginRequest.getUid() == 0) {
            log.error("参数校验失败，该参数为：{}", GsonUtil.toJson(quitOriginRequest));
            return ResultUtils.error(ErrorCode.PARAMS_ERROR, "Invalid oid and uid");
        }

        try {
            originUserService.quitOriginUser(quitOriginRequest.getOid(), quitOriginRequest.getUid());
            return ResultUtils.success(true);
        } catch (Exception e) {
            return ResultUtils.error(ErrorCode.SYSTEM_ERROR, "Failed to quit origin user");
        }
    }
