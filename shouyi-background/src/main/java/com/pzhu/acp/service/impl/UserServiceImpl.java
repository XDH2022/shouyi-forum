...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    @Override
    @Transactional(rollbackFor = {RuntimeException.class, BusinessException.class})
    public Boolean userReset(User user, String code) {
        // 1. Check if the email exists in the database
        QueryWrapper<User> userQueryWrapper = new QueryWrapper<>();
        userQueryWrapper.eq("email", user.getEmail());
        User existingUser = userMapper.selectOne(userQueryWrapper);
        if (existingUser == null) {
            log.warn("该邮箱不存在，该用户参数为：{}", GsonUtil.toJson(user));
            throw new BusinessException(ErrorCode.NO_EXISTED_EMAIL);
        }
        ...
        if (!code.equals(redisCode)) {
            log.warn("验证码错误，重置密码失败，该code参数为：{}", GsonUtil.toJson(code));
            throw new BusinessException(ErrorCode.ERROR_EMAIL_CODE);
        }
        ...
        if (operationNum == OperationConstant.OPERATION_NUM) {
            log.warn("重置密码失败,该用户参数为:{}", GsonUtil.toJson(user));
            throw new BusinessException(ErrorCode.UPDATE_ERROR);
        }

        // 4. Delete the verification code from Redis
        redisTemplate.delete(key);
