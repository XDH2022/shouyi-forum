          ...
          <el-card class="comment">
            ...
            <div class="comment-info" v-for="(item,index) in commentInfo?.items" :key="index">
              ...
              <div class="comment-main">
                ...
                <el-divider direction="vertical"/>
                <span style="cursor: pointer" @click="addReply(item.id)">回复</span>
                <div class="reply-input" v-show="replyInputState[item.id]">
                  <el-input
...
const replyInputState = reactive<ReplyInputState>({});

let activeCommentId : number = -1; // 追踪当前激活的输入框

const addReply = (commentId: number) => {
  if (activeCommentId !== -1) {
    console.log(activeCommentId)
    console.log(replyInputState)
    replyInputState[activeCommentId] = false; // 如果有激活的输入框，将其状态设置为false
  }
  replyInputState[commentId] = true; // 设置点击评论的输入框状态为true
  activeCommentId = commentId; // 更新当前激活的输入框
};
...
const doReply = async (item: any) => {
  ...
  addReplyReq.did = item.did
  addReplyReq.replyId=userStore.currentUser.userId
